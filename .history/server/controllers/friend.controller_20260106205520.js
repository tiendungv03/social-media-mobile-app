...
// 1. Gửi lời mời kết bạn (Follow)
export const sendFriendRequest = async (req, res) => {
  try {
    const { targetId } = req.params; // ID người mình muốn follow
    const { currentUserId } = req.body; // ID của mình (Gửi từ App lên)

    if (!currentUserId) return res.status(400).json({ message: "Thiếu ID người gửi" });
