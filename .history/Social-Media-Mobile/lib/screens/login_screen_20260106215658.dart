...
import 'package:flutter/material.dart';
// ❌ Bỏ import shared_preferences đi vì AuthService đã lo rồi
import '../main.dart'; // Import biến toàn cục authService
import 'home_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      // 2. Gọi API Login qua Service (Service đã tự lưu Token và ID vào máy rồi)
      final data = await authService.login(
        _usernameCtl.text.trim(),
        _passwordCtl.text.trim(),
      );
      ...
      if (data != null) {
        // 3. Lấy User ID để chuyển trang
        // Logic lấy ID thông minh của bạn vẫn giữ nguyên
        final userObj = data['user'];
        final userId = userObj?['_id'] ?? userObj?['id'];
      ...
      // Nếu data null thì authService đã in lỗi, UI không cần làm gì thêm hoặc hiện báo lỗi chung
  ...
  // ... (Phần UI bên dưới giữ nguyên y hệt code của bạn)
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // ... (Các phần UI phụ như Forgot Password, Sign up bạn giữ nguyên như cũ nhé) ...
              ],
            ),
          ),
        ),
