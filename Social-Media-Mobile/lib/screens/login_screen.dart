...
import 'package:flutter/material.dart';
import '../main.dart'; // Import để dùng authService
import 'home_screen.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  // --- LOGIC ĐĂNG NHẬP (Đã sửa lỗi Null Safety) ---
  Future<void> _login() async {
    setState(() {
      _loading = true;
      _error = null;
    ...
    try {
      ...
      // Lưu ý: data có thể là null nếu lỗi mạng
      final data = await authService.login(
        _usernameCtl.text.trim(),
        _passwordCtl.text.trim(),
      );
      ...
      // 👇 SỬA LỖI Ở ĐÂY: Kiểm tra data có tồn tại không rồi mới dùng
      if (data != null) {
        // Lấy ID để chuyển trang
        final userObj = data['user'];
        // Lấy _id hoặc id an toàn
        final userId = userObj != null ? (userObj['_id'] ?? userObj['id']) : null;
        ...
        if (userId != null) {
          ...
          setState(() => _error = "Lỗi: Không tìm thấy ID người dùng");
        }
      } else {
        // Nếu data là null thì authService đã in lỗi rồi
        // Ở đây mình chỉ cần báo lỗi chung chung nếu chưa có thông báo
        if (_error == null) setState(() => _error = "Đăng nhập thất bại");
      }
    } catch (e) {
      print("❌ Lỗi UI Login: $e");
      setState(() {
  ...
  InputDecoration _inputDecoration(String label) {
    return InputDecoration(
      ...
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: Color(0xFFBDBDBD), width: 0.7)),
    );
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                const SizedBox(height: 24),
                // Logo
                const Text('Mini Instagram', style: TextStyle(fontFamily: 'Billabong', fontSize: 42, color: Colors.black, letterSpacing: 1.2)),
                const SizedBox(height: 32),
                ...
                // 👇 CÁC PHẦN UI PHỤ ĐÃ ĐƯỢC KHÔI PHỤC 👇
                ...
                Row(
                  children: [
                    ...
                    const Padding(
                      padding: EdgeInsets.symmetric(horizontal: 8),
                      child: Text('OR', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w600)),
                    ),
                    Expanded(child: Container(height: 0.5, color: Colors.grey.shade400)),
                  ],
                ),
                ...
                TextButton.icon(
                  ...
                  label: const Text('Log in with Google', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.w600, fontSize: 14)),
                ),
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        ...
                        child: const Text('Sign up.', style: TextStyle(color: Color(0xFF3797EF), fontWeight: FontWeight.w600, fontSize: 13)),
                      ),
                    ],
                  ),
                ),
